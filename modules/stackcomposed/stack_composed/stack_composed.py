...
def run(
    ...
    Image.wrapper_shape = (
        int((max_y - min_y) / Image.wrapper_y_res),
    ...
    if len(images_files) != len(images):
        ...
        print(
            "  images to process: {0} (filtered in the range dates)".format(len(images))
        )
    else:
        print("  images to process: {0}".format(len(images)))
    print("  band(s) to process: {0}".format(",".join([str(b) for b in bands])))
    print(
        "  pixels size: {0} x {1}".format(
            round(Image.wrapper_x_res, 1), round(Image.wrapper_y_res, 1)
        )
    )
    print(
        "  wrapper size: {0} x {1} pixels".format(
            ...
            if band > image.n_bands:
                print(
                    "\n\nError: the image '{0}' don't have the band {1} needed to process\n".format(
                        image.file_path, band
                    )
                )
                exit(1)
        if round(image.x_res, 1) != round(Image.wrapper_x_res, 1) or round(
            image.y_res, 1
        ) != round(Image.wrapper_y_res, 1):
            print(
    ...
    for band in bands:
        ...
        if os.path.isdir(output):
            output_filename = os.path.join(
                output, "stack_composed_{}_band{}.tif".format(stat, band)
            )
        elif output.endswith((".tif", ".TIF")) and os.path.isdir(
        ...
        if output_type is None:
            ...
            if output_type == "uint16":
                gdal_output_type = gdal.GDT_UInt16
            if output_type == "uint32":
                gdal_output_type = gdal.GDT_UInt32
        ...
        outRaster.SetGeoTransform(
            (
                Image.wrapper_extent[0],
                Image.wrapper_x_res,
                0,
                Image.wrapper_extent[1],
                0,
                -Image.wrapper_y_res,
            )
        )
