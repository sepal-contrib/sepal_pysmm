...
def script():
    ...
    def nodata_validator(s):
        try:
            ...
            try:
                if "or" in s:
                    conditions = [
                        split_condition(str_cond) for str_cond in s.split("or")
                    ]
                    return conditions
                else:
    ...
    parser.add_argument(
        "-stat", type=str, help="Statistic for compute the composed", required=True
    )
    parser.add_argument(
        "-bands", type=str, help="Band or bands to process, e.g. 1,2,3", required=True
    )
    parser.add_argument(
        "-nodata",
        type=nodata_validator,
        required=False,
    ...
    parser.add_argument(
        ...
        type=str,
        dest="output",
        help="output directory and/or filename for save results",
        default=os.getcwd(),
    )
    parser.add_argument(
        "-ot",
        type=str,
        dest="output_type",
        help="Output data type for results",
        required=False,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Chunks size for parallel process",
        required=False,
    )
    parser.add_argument(
        "-start",
        type=date_validator,
        dest="start_date",
        help="Initial date for filter data, format YYYY-MM-DD",
        required=False,
    )
    parser.add_argument(
        "-end",
        type=date_validator,
        dest="end_date",
        help="End date for filter data, format YYYY-MM-DD",
        required=False,
    )
    parser.add_argument(
        "inputs", type=str, help="Directories or images files to process", nargs="*"
    )
