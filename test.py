...
class DrawerItem(sw.ListItem):
    ...
    def __init__(self, title, icon=None, card=None, href=None, model=None, bind_var=None, **kwargs):
        self.rt = rt
        icon = icon if icon else 'fa-regular fa-folder'
        children = [v.ListItemAction(children=[v.Icon(class_='white--text', children=[icon])]), v.ListItemContent(children=[v.ListItemTitle(class_='white--text', children=[title])])]
        kwargs['link'] = True
        ...
        self.alert_badge = v.ListItemAction(children=[v.Icon(children=['fa-solid fa-circle'], x_small=True, color='red')])
        if model:
            if not bind_var:
...
class NavDrawer(sw.NavigationDrawer):
    ...
    def __init__(self, items=[], code=None, wiki=None, issue=None, **kwargs):
        ...
        code_link = []
        if code:
            item_code = DrawerItem(ms.widgets.navdrawer.code, icon='fa-regular fa-file-code', href=code)
            code_link.append(item_code)
        if wiki:
            item_wiki = DrawerItem(ms.widgets.navdrawer.wiki, icon='fa-solid fa-book-open', href=wiki)
        ...
        children = [v.List(dense=True, children=self.items), v.Divider(), v.List(dense=True, children=code_link)]
