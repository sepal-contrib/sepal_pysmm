...
def bindAoiMethod(method_widget, list_input, obj, m, dc, selection_method, alert_box):
    ...
    def on_change(
        widget, event, data, list_input, obj, m, dc, selection_method, alert_box
    ):
        ...
        if widget.v_model == "Country boundaries":  # country selection
            ...
            def asset_change_event(widget, event, data, aoi):
                ...
                toggle_inputs(
                    [w_asset_column, w_asset_field], [w_asset_column, w_asset_field]
                )
...
def handle_draw(dc, obj, variable, output=None):
    ...
    dc.on_draw(partial(on_draw, obj=obj, variable=variable, output=output))
...
def bindAoiProcess(btn, io, m, dc, list_method):
    ...
    btn.on_event("click", partial(on_click, io=io, m=m, dc=dc, list_method=list_method))
