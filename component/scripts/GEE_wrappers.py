...
class GEE_pt(object):
    ...
    def _s1_track_ts(
        ...
        def tolin_dual(image):
            ...
            if dual_pol == True:
                lin = ee.Image(10).pow(
                    tmp.divide(10)
                )  # .select(['constant', 'constant_1'], ['VV', 'VH'])
            else:
        ...
        def createAvg(image):
            ...
            return ee.Feature(
                None,
                {"result": reduced_img_data, "count": pcount, "totcount": totcount},
            )

        #  filter for track
        if dual_pol == True:
            gee_s1_track_fltd = filtered_collection.filterMetadata(
        ...
        if tempfilter == True:
            ...
            if dual_pol == True:
                ...
                gee_s1_dspckld_vh = gee_s1_dspckld_vh.map(todb).select(
                    ["constant"], ["VH"]
                )
            if dual_pol == False:
                gee_s1_track_fltd = gee_s1_dspckld_vv.combine(
                    gee_s1_track_fltd.select("angle")
                ).combine(gee_s1_track_fltd.select("lia"))
            else:
                gee_s1_track_fltd = (
                    gee_s1_dspckld_vv.combine(gee_s1_dspckld_vh)
        ...
        vv_sig0 = 10 * np.log10(
            np.array(
                [x["properties"]["result"]["VV"] for x in tmp["features"]],
                dtype=np.float,
            )
        )
        ...
        ge_dates = np.array(
            [
                dt.datetime.strptime(x["id"][17:32], "%Y%m%dT%H%M%S")
        ...
        if dual_pol == True:
            # get vh
            vh_sig0_lin = np.array(
                [x["properties"]["result"]["VH"] for x in tmp["features"]],
                dtype=np.float,
            )
            vh_sig0 = 10 * np.log10(vh_sig0_lin)
            if masksnow == True:
                snowref = 10 * np.log10(
                    np.mean(vh_sig0_lin[vh_sig0_lin > np.percentile(vh_sig0_lin, 5)])
                )
        ...
        if returnLIA == True:
            # get lia
            lia = np.array(
                [x["properties"]["result"]["lia"] for x in tmp["features"]],
                dtype=np.float,
            )
        else:
            # get angle
            lia = np.array(
                [x["properties"]["result"]["angle"] for x in tmp["features"]],
                dtype=np.float,
            )
        ...
        val_count = np.array(
            [
                np.float(x["properties"]["count"]["VV"])
                / np.float(x["properties"]["totcount"]["constant"])
                for x in tmp["features"]
            ],
            dtype=np.float,
        )
        ...
        if dual_pol == True:
            return pd.DataFrame(
                {"sig0": vv_sig0, "sig02": vh_sig0, "lia": lia}, index=ge_dates
            )
        else:
    ...
    def extr_LC(self, prod="Globcover"):
        ...
        if prod == "Globcover":
            lc_image = ee.Image("ESA/GLOBCOVER_L4_200901_200912_V2_3").select(
                "landcover"
            )
        elif prod == "Corine":
    ...
    def extr_MODIS_MOD13Q1(self):
        ...
        EVI = np.array(
            [x["properties"]["result"]["EVI"] for x in tmp["features"]], dtype=np.float
        )

        ge_dates = np.array(
            [dt.datetime.strptime(x["id"], "%Y_%m_%d") for x in tmp["features"]]
        )
    ...
    def extr_GLDAS_SM(self, yearlist=None):
        ...
        def createAvg(image):
            ...
            reduced_img_data = image.reduceRegion(
                ee.Reducer.median(), gee_roi, 30, tileScale=4
            )
            return ee.Feature(None, {"result": reduced_img_data})

        if yearlist == None:
            # yearlist = range(1987,2018)
            yearlist = list(range(2011, 2018))
        ...
        for iyear in yearlist:
            ...
            # load collection
            if iyear < 2000:
                GLDAS_collection = ee.ImageCollection(
            ...
            GLDAS_collection = GLDAS_collection.filterDate(
                str(iyear) + "-01-01", str(iyear) + "-12-31"
            )
            GLDAS_collection = GLDAS_collection.filter(
                ee.Filter.calendarRange(16, 18, "hour")
            )
            ...
            SM = np.array(
                [
                    x["properties"]["result"]["SoilMoi0_10cm_inst"]
                    for x in tmp["features"]
                ],
                dtype=np.float,
            )
            ...
            ge_dates = np.array(
                [dt.datetime.strptime(x["id"], "A%Y%m%d_%H%M") for x in tmp["features"]]
            )
    ...
    def extr_terrain(self):
        roi = ee.Geometry.Point(self.lon, self.lat).buffer(self.buffer)
        elev = (
            ee.Image("CGIAR/SRTM90_V4").reduceRegion(ee.Reducer.median(), roi).getInfo()
        )
        aspe = (
            ee.Terrain.aspect(ee.Image("CGIAR/SRTM90_V4"))
            .reduceRegion(ee.Reducer.median(), roi)
            .getInfo()
        )
        slop = (
    ...
    def extr_S1(
        ...
        def s1_simplyfy_date(image):
            return image.set(
                "system:time_start",
                ee.Date(ee.Date(image.get("system:time_start")).format("Y-M-d")),
            )
        ...
        if asc == False:
            # Select only acquisition from descending tracks
            gee_s1_filtered = gee_s1_filtered.filter(
        ...
        if ssmcor is not None:
            ...
            ssm_vv_cor = s1_plus_RefSM.select(["VV", "constant"]).reduce(
        ...
        for track_nr in available_tracks:
            track_nr = (
                track_nr.item()
            )  # value corrected, .item() to convert most NumPy values to a native Python type
            out_dict[str(track_nr)] = self._s1_track_ts(
                gee_s1_filtered,
                track_nr,
                dual_pol,
                varmask,
        ...
        print(
            (
                "Time-series extraction finished in "
                + "{:10.2f}".format(toc - tic)
                + "seconds"
            )
        )
    ...
    def extr_SM(
        self, tracknr=None, masksnow=False, tempfilter=True, calc_anomalies=False
    ):
        ...
        modelpath = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "SVRmodel.p"
        )
        ...
        empty = 0
        for track_id in list(s1_ts.keys()):
        ...
        for track_id in list(s1_ts.keys()):
            ...
            if ts_length < 10:
                print(
                    (
                        "Estimation for track #"
                        + str(track_id)
                        + " not possible - time-series too short"
                    )
                )
                continue

            temp_ts1_lin = np.power(10, temp_ts1 / 10.0)
            temp_ts2_lin = np.power(10, temp_ts2 / 10.0)
            ...
            )  # , aspe, slop, elev, gldas_series.mean()])

            fmat_tmp2 = np.hstack(
                (
                    (temp_ts1_lin - meanVV).values.reshape(ts_length, 1),
            ...
            ssm_estimated_tmp = pd.Series(
                (diff_ssm_estimated_tmp + avg_ssm_estimated_tmp) * 100,
                index=dates,
                name="S1",
            )
            ssm_estimated_tmp = ssm_estimated_tmp[
                ~ssm_estimated_tmp.index.duplicated(keep="first")
            ]
            ...
            if calc_anomalies == True:
                ...
                gldas_s1 = pd.concat(
                    [ssm_estimated_tmp, gldas_ts_matched["GLDAS"]], axis=1, join="inner"
                ).dropna()
                lin_fit = LinearRegression()
                lin_fit.fit(
                    gldas_s1["GLDAS"].values.reshape(-1, 1), gldas_s1["S1"].values
                )
                # calibrate GLDAS
                ...
                anom_estimated_tmp = pd.Series(
                    [
                        ssm_estimated_tmp[dateI] - gldas_clim["S1"][dateI.dayofyear]
                        for dateI in ssm_estimated_tmp.index
                    ],
        ...
        if calc_anomalies == False:
            ...
            return pd.DataFrame(
                {"S1SM": sm_estimated, "ANOM": anom_estimated}, index=sm_estimated.index
            )
...
class GEE_extent(object):
    ...
    def get_S1(
        ...
        if descending == True:
            # Consider only image from descending orbits
            gee_s1_filtered = gee_s1_filtered.filter(
                ee.Filter.eq("orbitProperties_pass", "DESCENDING")
            )
