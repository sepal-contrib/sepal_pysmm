...
def get_ts(
    ...
    for iloc in loc:
        ...
        if names is not None:
            ...
            iname = None
        print(
            "Estimating surface soil moisture for lon: "
            + str(lon)
            + " lat: "
            + str(lat)
        )
        ...
        sm_ts = gee_pt_obj.extr_SM(
            tracknr=tracknr, masksnow=masksnow, calc_anomalies=calc_anomalies
        )
        ...
        if isinstance(sm_ts, pd.Series) or isinstance(sm_ts, pd.DataFrame):
            ...
            if create_plots == True:
                if calc_anomalies == False:
                    ...
                    y0 = [0, 0]
                    (line2,) = ax.plot(x0, y0, color="k", linestyle="--", linewidth=0.2)
