...
def raw_to_processed(image):
    ...
    image_name = ntpath.basename(image)
    out_close_path = os.path.dirname(os.path.abspath(image)).replace(
        "0_raw", "1_processed"
    )
...
def run_filter(process_path, alert):
    ...
    if len(image_files) > 0:
        ...
        alert.add_msg(
            f'Error: The folder: "{folder}" is empty, please \
         make sure you have processed and downloaded the images.',
            type_="error",
        )
        return
    try:
